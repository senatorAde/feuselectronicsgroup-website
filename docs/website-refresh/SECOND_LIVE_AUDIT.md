...
## Legal navigation follow-up — 2026-09-08

- Verified clean website branch `remediation/second-live-audit`, open PR #4, and exact local/PR head `7a7a5b34551a79ba20ce00fd0bfda32ffe9640a5` before editing. This follow-up is scoped separately from the historical implementation record below.
- Removed only the two footer `(draft)` spans: **Privacy notice** → `/legal/privacy`; **Terms of use** → `/legal/terms`. Search across website JSX/JS found no other qualified navigation labels in navbar or onboarding. In-sentence links, legal-page metadata/content, unapproved/non-binding disclosures, publication dates, and all pinned public/historical evidence figures are unchanged.
- Rendered regression now exercises the actual shared layout (navbar/footer) on Home, Get Started and adoption Contact, asserting exact footer labels and hrefs and checking all rendered legal links. Both legal pages must retain the published-draft, counsel-approval-pending, no-contract and original-publication-date disclosures; Contact retains its draft-pending-approval notice.
- Actual validation: **`npm test` passed, 15/15 tests**, both existing validators passed (**73 files / 22 capability rows**); **`npm run build` completed**, Vite 5.4.21, **2013 modules**, **26.48s**. Existing React `fetchPriority` and stale Browserslist warnings remain. Shared-terminal interruptions were not counted as passing runs; the successful build and Edge run were isolated processes. `git diff --check` and changed-source editor diagnostics passed.
- Local built-preview Edge audit: **28/28 route/viewport checks**, **25/25 native pointer checks**, including **8/8 legal-link clicks** (both links from Home and Get Started at **1440 × 1000** and **390 × 844**). Exact labels, destinations, page headings and retained draft/non-binding disclosures passed. Clicks were trusted native CDP mouse input in installed headless Edge, not DOM dispatch or physical-device/touch testing. **27 asset URLs**, **0 local HTTP errors**, **0 non-GET requests**. External requests were blocked. Representative desktop Privacy and narrow Terms screenshots were visually reviewed. Report/screenshots: **%TEMP%/feus-web-pointer-audit-IEr28E**, outside the repository.
- Local verification only: **no fresh live-site or cloud verification**, inquiries, authentication, merge or deployment. This follow-up is authorized for commit/push to the same PR #4; earlier no-commit/no-push statements below describe the original audit stage, not this follow-up.
